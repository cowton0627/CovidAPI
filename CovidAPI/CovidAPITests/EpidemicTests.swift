...
final class EpidemicTests: XCTestCase {
    ...
    func testViewModelSortsBySeverityThenDate() {
        let newestWatch = Epidemic(
            headline: "泰國 第一級注意",
            effective: date,
            description: "內容",
            severityLevel: 1
        )
        let olderWarning = Epidemic(
            headline: "日本 第三級警告",
        ...
        let repository = EpidemicRepository(
            apiClient: APIClientStub(result: .success([olderWarning, newestWatch])),
            cache: CacheStub()
        )
        let viewModel = EpidemicListViewModel(
            repository: repository,
        ...
        XCTAssertEqual(viewModel.visibleEpidemics.first, newestWatch)
